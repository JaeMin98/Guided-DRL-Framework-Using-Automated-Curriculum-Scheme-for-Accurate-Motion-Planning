...
wandb.init(project='SAC_robotic_arm_Orientation')
wandb.run.name = 'SAC_ROBOTARM_ORIENTATION'
wandb.run.save()
...
def Run_Training():
    ...
    for i_episode in itertools.count(1):
        ...
        env.reset()
        state, ___, ____, ____ = env.observation()

        while not done:
            if Config.start_steps > total_numsteps:
                random_values = np.random.uniform(-1, 1, size=6)
