...
class plot():
    def get_plot_of_algorithm_type(self, feature, models, cluster_center):
        num_row, num_col = 3, 6
        ...
        for i in range(len(models)):
            ...
            current_row = (i+num_col) // num_col

            if current_row > 0:
                axes = ax[i+current_row*num_col]
            else:
                axes = ax[i]

            s_score = round(silhouette_score(X, labels),4)
            c_score = int(calinski_harabasz_score(X, labels))
            d_score = round(davies_bouldin_score(X, labels),4)
            # s_score = 1.0
            # c_score = 1.0
            # d_score = 1.0
                        # 표 데이터 준비
            ...
            # 표 생성
            table = axes.table(cellText=data, loc='center', cellLoc='center', colLabels=["silhouette", "calinski", "Davies"], colColours=['lightgray']*3)
        ...
        for i in range(len(models)):
            ...
            current_row = (i+num_col*2) // num_col

            ax[i+current_row*num_col].get_xaxis().set_visible(False)
            ax[i+current_row*num_col].get_yaxis().set_visible(False)
            ax[i+current_row*num_col].spines['top'].set_visible(False)
            ax[i+current_row*num_col].spines['right'].set_visible(False)
            ax[i+current_row*num_col].spines['bottom'].set_visible(False)
            ax[i+current_row*num_col].spines['left'].set_visible(False)

            if current_row > 0:
                axes = fig.add_subplot(3, 6, i+1+current_row*num_col, projection='3d')
            else:
                axes = fig.add_subplot(3, 6, i+1, projection='3d', sharex=ax, sharey=ax)
            ...
            axes.scatter(x, y, z, c=labels, s=0.4, alpha =0.7, cmap='rainbow')
...
if __name__== "__main__" :
    ...
    algorithom_models = [KMeans[0], BisectingKMeans[0], MiniBatchKMeans[0], Birch[0], GaussianMixture[0], BayesianGaussianMixture[0]]
    cluster_center = [KMeans[1], BisectingKMeans[1], MiniBatchKMeans[1], Birch[1], GaussianMixture[1], BayesianGaussianMixture[1]]

    P = plot()
    # P.get_plot_of_algorithm_type(feature, algorithom_models, cluster_center)

    curriculum_models = [KMeans, BisectingKMeans, MiniBatchKMeans]
    P.get_plot_of_curriculum_unit(feature, curriculum_models, original_row)

    curriculum_models = [Birch, GaussianMixture, BayesianGaussianMixture]
    P.get_plot_of_curriculum_unit(feature, curriculum_models, original_row)