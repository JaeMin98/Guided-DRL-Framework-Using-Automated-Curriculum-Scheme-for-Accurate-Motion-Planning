...
# CSV 파일 열기
with open('SuperVised_(2023-05-24_12-59)\predict_csv_X_train.csv', 'r') as file:
    ...
    for row in reader:
        ...
        row_dis_delta = [row[3],row[4],row[5]]
        row_dis_delta = list(map(float, row_dis_delta))

        rows.append(row_temp)
        row_XYZ.append(row_xyz)
        
        row_DD.append(row_dis_delta)
...
for i in range(len(RunT)):
    Kmeans_color.append(colors[kmeans.predict([[row_DD[i][0],row_DD[i][1],row_DD[i][2]]])[0]])
...
for i in range(len(x)):
    color_index.append(kmeans.predict([[row_DD[i][0],row_DD[i][1],row_DD[i][2]]])[0])
