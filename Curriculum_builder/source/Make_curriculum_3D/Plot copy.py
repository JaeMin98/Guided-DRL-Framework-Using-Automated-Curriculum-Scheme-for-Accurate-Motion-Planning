...
class plot():
    def get_plot_of_algorithm_type(self, feature, models, cluster_center, cluster_label):
        num_row, num_col = 3, 6
        ...
        for i in range(len(models)):
            ...
            if(plot_name == "GaussianMixture" or plot_name =="BayesianGaussianMixture"):
                label = models[i].predict(feature)
            else:
                label = models[i].labels_
            ...
            print(plot_name)
            for C in center:
                distance = euclidean_distance_3d(C[0], C[1],C[2], -3, -3, -3)
                print(C, distance)
            ...
            for L in label:
                try:
                    main_color.append(colors[cluster_sequence[L]-1])
                except:
                    pass
#--------------------------------------------------------------------------------------------------------------------------------
            df = pd.DataFrame({'X1': X[:, 0], 'X2': X[:, 1], 'X3': X[:, 2], 'Cluster': label})

            ax = fig.add_subplot(num_row, num_col, i + 1, projection='3d')  # 3D 서브플롯 생성
            ax.scatter(X[:, 0], X[:, 1], X[:, 2], c=main_color, s=0.5, cmap='rainbow')  # 3D 산점도 표시
    ...
    def get_plot_of_curriculum_unit(self, feature, models, original_row):
        ...
        for i in range(len(models)):
            ...
            for center in cluster_centers:
                ...
                print(len(center))
                distance = euclidean_distance_3d(center[0], center[1], center[2] -3, -3, -3)
                distance_list.append(distance)
            cluster_sequence = rank_list(distance_list)
...
if __name__== "__main__" :
    ...
    P = plot()
    P.get_plot_of_algorithm_type(feature, algorithom_models, cluster_center, cluster_label)
